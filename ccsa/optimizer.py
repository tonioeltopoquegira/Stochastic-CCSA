...
from .params import MMA_RhoParams, MMA_SigmaParams, update_rho
from .asymptote import AsymptoteUpdater
from .dual import DualSubproblemBuilder
    ...
    def step(self):
        ...
        for inner in range(self.max_inner):
            ...
            x_candidate, tilde_f, tilde_gc, w_val, val_extra = \
                builder.reconstruct_xcandidate_from_y(y_opt)

            # true f,g at candidate
            if self.df is None:
            ...
            gcur = np.atleast_1d(self.g(x_candidate)).astype(float) if m > 0 else np.zeros(0)
        ...
        # ===== OUTER UPDATES =====
        L_new, U_new = self.asym.update(
            x_km1=x_k, x_kp1=x_best, L=self.L.copy(), U=self.U.copy()
        )
        ...
        self.metrics.setdefault("x_history", []).append(x_best.copy())
        self.metrics.setdefault("sigma_history", []).append(self.asym.sigma.copy())
        self.metrics.setdefault("rho_history", []).append(self.rho)
        self.metrics.setdefault("rho_c_history", []).append(self.rho_c.copy())

        self.metrics.setdefault("cumulative_weighted_evals_history", []).append(
            float(self.metrics["weighted_evals"])
        )
        ...
        self.history.setdefault("rho_c", []).append(self.rho_c.copy())
        self.history.setdefault("weighted_evals", []).append(
            float(self.metrics["weighted_evals"])
        )
