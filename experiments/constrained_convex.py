...
def stoch_convex_con_exp(
    ...
    # ANALYTICAL EXPECTED OPTIMUMS (for verification and plotting)
    ...
    print("Analytic expected optimum:")
    ...
    g_uncon = -b
    print(f"Unconstrained optimum: E[f(x_uncon)]= {val_uncon:.6g}, g(x_uncon)={g_uncon:.6g}")

    # Sampling Closure used by all function/grad calls
    def sample_xi():
        return rng.randn(n) * noise

    # Single-call noisy oracle: returns a function f_and_grad(x, xi=None, grad=None)
    def make_noisy_f_and_grad(A, sample_xi):
        ...
        def f_and_grad(x, xi=None, grad=None):
            ...
            # If grad is True (python-optimizer request that wants value+grad)
            if grad is True:
                return val, grad_val
    ...
    # Expected (closed-form) objective for plotting
    def expected_f(x):
    ...
    x_al, lam_al, hist_al = adam_augmented_lagrangian(
        ...
        dg=lambda xx: c,                      # analytic gradient of g w.r.t x 
    ...
    # NLopt-MMA - here still using its own f_and_grad interface
    mma_results = []
    ...
    # samples per iter, in the paper they use more than 1 at times!
    ...
    # Prob not strictly necessary helper
    ...
    # Run another custom CCSA, potentially with different args (e.g. quadratic surrogates, Adam curvature updates)
    oracle = make_noisy_f_and_grad(A, sample_xi)
