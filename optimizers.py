...
from utils import evaluate   # <- make sure this import is correct
...
class CCSAOptimizer(torch.optim.Optimizer):
    ...
    def optimize_training(self,
                          ...
                          device,
                          epochs: int,
                          test_loader=None) -> Tuple[List[float], List[float], dict]:
        ...
        opt.set_param("sigma_min", float(group.get("sigma_min", 0.0)))
        opt.set_param("inner_maxeval", int(group.get("max_inner_eval", 200)))
        ...
        def loss(x, grad):
            ...
            if grad.size > 0:
                ...
                if batches_per_epoch > 0 and outer_calls % batches_per_epoch == 0:
                    ...
                    if verbose:
                        msg = (f"[CCSA][epoch {epoch_idx}] "
                               f"train_eval_loss={tr_loss:.4f} train_eval_acc={tr_acc:.4f}")
        ...
        return outer_batch_losses, cumulative_weighted_evals, logs
